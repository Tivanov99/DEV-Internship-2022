#pragma once
class GlobalConstants
{
public:
	static const int _nCityNameSize = 32;
	static const int _nMinCityNameSize = 4;

	static const int _nCityAreaNameSize = 32;
	static const int _nMinCityAreaNameSize = 4;

	static const int _nPersonFirstNameSize = 32;
	static const int _nPersonFirstNameMinSize = 3;

	static const int _nPersonSecondNameSize = 32;
	static const int _nPersonSecondNameMinSize = 4;

	static const int _nPersonLastNameSize = 32;
	static const int _nPersonLastNameMinSize = 4;

	static const int _nPersonUcnSize = 16;
	static const int _nPersonUcnMinSize = 10;

	static const int _nPersonAddressSize = 256;
	static const int _nPersonAddressMinSize = 16;


	static const int _nPhoneNumberSize = 16;
	static const int _nPhoneNumberMinSize = 10;

	static const int _nColumnWidth = 120;

	static const int _nPhoneTypeMinLenght = 4;
	static const int _nPhoneTypeMaxLenght = 16;

};


